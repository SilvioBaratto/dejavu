...
def _is_transient(exc: BaseException) -> bool:
    ...
    try:
        import anthropic  # present in production; may be absent in isolated test envs
...
async def resilient_session(
    ...
    *,
    ...
    but prompt-caching makes those calls cheap (cache-hit only).  Per-turn retry
    would require modifying the runner loop, which is out of scope (Cycle 4 frozen).
    """
